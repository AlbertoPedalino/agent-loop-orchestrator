...
RESOURCE_ROOT = PROJECT_ROOT / "agent" / "resources"


def test_default_subagents_load_with_read_only_roles() -> None:
    configs = load_subagents_config(RESOURCE_ROOT / "configs" / "subagents.default.yaml")

    assert set(configs) == {"planner", "implementer", "fixer", "reviewer"}
    assert configs["planner"].prompt_template == RESOURCE_ROOT / "prompts" / "planner.md"
...
def test_removed_max_turns_fails_instead_of_being_ignored(tmp_path: Path) -> None:
    config_path = tmp_path / "configs" / "old.yaml"
    prompt_path = tmp_path / "prompts" / "planner.md"
    config_path.parent.mkdir()
    prompt_path.parent.mkdir()
    prompt_path.write_text("# Planner", encoding="utf-8")
...
DEFAULTS_PATH = RESOURCE_ROOT / "configs" / "subagents.default.yaml"
...
def test_target_overlay_adds_skills_and_inherits_the_rest(tmp_path: Path) -> None:
    ...
    assert configs["implementer"].permission_mode == "acceptEdits"
    assert configs["implementer"].prompt_template == RESOURCE_ROOT / "prompts" / "implementer.md"

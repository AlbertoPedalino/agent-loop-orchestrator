...
A phase may declare `skills` (`name` or `plugin:name`); an entry can also be a `{name, args}` mapping to request a skill mode (e.g. `{name: caveman:caveman, args: wenyan-ultra}`), passed through verbatim to the skill invocation. With the Claude backend, the CLI discovers skill content natively from the active target repository, user scope, or plugins. If an ignored repository-local skill exists only in the source checkout and is therefore absent from an isolated worktree, the orchestrator inlines that skill into the phase prompt instead. With the Codex backend, which has no skill loader, every `SKILL.md` body is inlined into the phase prompt, resolved from the same sources: the target repository, the user scope (`~/.claude/skills/`), or — for `plugin:name` — the installed plugin snapshot listed in `~/.claude/plugins/installed_plugins.json`. A skill that cannot be resolved for Codex fails the phase with an explicit error.
